...
\newcommand{\dletin}[3]{\text{let } #1 = #2 \text{ in } #3} % sin tipo
...
Vamos a definir un \textbf{Comando} como una expresión qeu se evalúa para
...
\begin{itemize}
    \item \textbf{Reescribir}: $\mustore{I}{V}$ es el store resultante de \textbf{pisar} $\mu(I)$ con $V$.
    \item \textbf{Extender}: $\emustore{I}{V}$ es el \textbf{store extendido} resultante de ampliar
