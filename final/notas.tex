...
\newcommand{\appTSust}[1]{S#1} % apply type sust
\newcommand{\tsust}[2]{#1/#2} % type sust
...
variables de tipos en tipos concretos, por ejemplo $\appTSust{s} = Int$.
...
\begin{enumerate}
    \item Una expresión de tipos $\sigma$ (escribimos $\appTSust{\sigma}$)
    \item Un término M (escribimos $\appTSust{M}$)
    \item Un contexto de tipado
    $\Gamma = \{ x_1 : \sigma_1, \dots, x_n : \sigma_n\}$
    \[
        \appTSust{\Gamma} \eqdef
        \{
            x_1 : \appTSust{\sigma_1}, 
            \dots,
            x_n : \appTSust{\sigma_n}
        \}
    \]
\end{enumerate}
...
\begin{itemize}
    \item El conjunto $\{ t \mid \appTSust{t} \neq t \}$ se llama \textbf{soporte}
    de $S$, representa las variables afectadas por $S$.
    \item Notamos $\{ \tsust{\sigma_1}{t_1}, \dots, \tsust{\sigma_n}{t_n} \}$
